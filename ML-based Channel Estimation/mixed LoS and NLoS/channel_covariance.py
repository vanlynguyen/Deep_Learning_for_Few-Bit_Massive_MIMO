...
theta_mean = (pi/3)*(2*np.random.uniform(size=K)-1)
theta_min = -pi/2
theta_max =  pi/2
Sk_denom = 1/(sqrt2*sigma*(1-np.exp(-sqrt2*np.pi/sigma)))

for k in range(K):
    Sk = lambda theta: np.exp(-sqrt2*np.abs(theta-theta_mean[k])/sigma)/Sk_denom
    ...
    for n in range(N):
        R_kn_real = lambda theta: np.cos(pi*n*np.sin(theta))* \
            np.exp(-sqrt2*np.abs(theta-theta_mean[k])/sigma)/Sk_denom
